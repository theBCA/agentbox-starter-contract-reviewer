...
# curl is here for OPERATOR use, not the app: it is the tool you reach for
# to demonstrate the egress boundary from inside the sandbox
# (`curl https://google.com` -> refused BY SECUREPROXY under an allowlist,
# vs. a DNS failure with no proxy at all). curl honours HTTP_PROXY/HTTPS_PROXY
# natively, which is exactly the path AgentBox configures, so it shows the
# real policy decision rather than a synthetic one. Installed at BUILD time,
# where the network is deliberately unrestricted -- a runtime `apt-get install`
# would go through Package Guard instead.
...
# contracts. Source, not a mount -- it is what this application IS.
...
HEALTHCHECK --interval=10s --timeout=10s --start-period=10s --retries=3 \
    CMD python -c "import socket; socket.create_connection(('127.0.0.1', 8082), timeout=8).close()" || exit 1
