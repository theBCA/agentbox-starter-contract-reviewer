...
(Python, Google ADK, routed through OpenRouter).
...
_BACKEND_MODULE = "app.backends.google"
_AGENT_TYPE = "google-adk"
...
class DeleteRedlinesRequest(BaseModel):
    contract_id: str
...
def _bundled_server_name() -> str:
    """The bridge name of this app's own bundled MCP server, the clause
    library (`mcp/clause-library/`).
    ...
    """
    app_id = (
        os.environ.get("AGENTBOX_APP_ID", "contract-reviewer").strip()
        or "contract-reviewer"
    )
    return f"{app_id}__clause-library"


@app.post("/mcp/find-standard-clause", response_model=BridgeCallResponse)
async def find_standard_clause_via_mcp(
    payload: FindClauseRequest,
) -> BridgeCallResponse:
    """Read the clause library through the bridge, as the agent's own
    `find_standard_clause` does. The server declares the operation read-only,
    so the bridge never holds it -- the same call the agent makes in step 3."""
    topic = payload.topic.strip()
    if not topic:
        raise HTTPException(status_code=400, detail="topic must not be empty")
    server = _bundled_server_name()
    data = await asyncio.to_thread(
        _mcp_call, server, "find_standard_clause", {"topic": topic}
    )
    return BridgeCallResponse(
        ok=True, bridge_server=server, bridge_tool="find_standard_clause", result=data
    )


@app.post("/mcp/save-redline", response_model=BridgeCallResponse)
async def save_redline_via_mcp(payload: SaveRedlineRequest) -> BridgeCallResponse:
    """Write a redline through the bridge: an ordinary write, allowed under
    the grant and recorded."""
    server = _bundled_server_name()
    data = await asyncio.to_thread(
        _mcp_call,
        server,
    ...
    return BridgeCallResponse(
        ok=True, bridge_server=server, bridge_tool="save_redline", result=data
    )


@app.post("/mcp/delete-all-redlines", response_model=BridgeCallResponse)
async def delete_all_redlines_via_mcp(
    payload: DeleteRedlinesRequest,
) -> BridgeCallResponse:
    """Ask to delete every redline of a contract -- and expect to be stopped.

    `delete_all_redlines` is classified DESTRUCTIVE by the bridge's own
    ...
    return BridgeCallResponse(
        ok=True, bridge_server=server, bridge_tool="delete_all_redlines", result=data
    )
